...
# Убираем @onready, будем получать узлы вручную в setup()

var map_data: Dictionary
var custom_tiles_path: String

func setup(data: Dictionary, base_path: String) -> void:
	# Получаем слои напрямую через get_node (работает даже до входа в дерево)
	...
	# Создаём общий TileSet
	var tile_set = TileSet.new()
	var type_to_source = {}

	# 1. Стандартные тайлы
	for tile_type in range(0, 11):
		var texture = generate_base_tile_texture(tile_type, data.get("tile_size", 64))
		var source_id = tile_set.add_source(TileSetAtlasSource.new())
		if source_id == -1:
			continue
		var atlas = tile_set.get_source(source_id)
		atlas.texture = texture
		atlas.texture_region_size = Vector2i(data.get("tile_size", 64), data.get("tile_size", 64))
	...
	if DirAccess.dir_exists_absolute(custom_tiles_path):
		...
		while file_name != "":
			if not dir.current_is_dir() and file_name.get_extension().to_lower() in ["png", "jpg", "jpeg", "bmp"]:
				var image = Image.load_from_file(custom_tiles_path.path_join(file_name))
				if image == null:
				...
				var source_id = tile_set.add_source(TileSetAtlasSource.new())
				if source_id == -1:
					push_error("Не удалось добавить кастомный тайл")
					continue
				var atlas = tile_set.get_source(source_id)
				if atlas == null:
					continue
